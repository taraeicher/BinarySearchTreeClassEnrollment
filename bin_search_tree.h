/*Program Description: Manipulate a singly-linked list of car structs.
...
int populate(FILE *stream, Tree *t);

//Construct the key from file data.

char *getKey(FILE *stream);

//Print tree contents to a file using a specified order.

int printToFile(Node *n, FILE *fp, int order);
